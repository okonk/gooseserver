...
// THE ONE BUNDLE EDITOR.HTML INLINES. Every sheet needs it, so it is on the page as a global,
// exactly as the deployed sidebar has it.
globalThis.GOOSE_SPRITES = {
  icons: { png: 'data:image/png;base64,ICONS', rects: { '1:1': [0, 0, 16, 16] } },
};

// AND THE TWO IT DOES NOT. parts and effects are fetched with google.script.run.include when a sheet
// asks for one, so in a test they live on the SERVER: makeServer's include() wraps whichever of
// these the caller asks for in the file format tools/SpriteBundle writes. The payload is rebuilt per
// call, so a test that reaches in and changes `rects` around one case still works — the same trick
// the old inlined fixture allowed.
//
// One usable body sprite, so the character preview has something to draw, and no effect frames at
// all — an effect preview that started a real interval would outlive the test. The tests that need
// frames supply their own.
const SERVED = {
  parts: { png: 'data:image/png;base64,PARTS', rects: { 'Bodies:1:idle-down': [0, 0, 48, 48] } },
  effects: { png: 'data:image/png;base64,EFFECTS', rects: {} },
};

// What include() returns for a bundle: the generated file, verbatim in shape. The literal is strict
// JSON, which is what app.js's parseBundle relies on — and 'the real files still look like this' is
// pinned separately, against sprites-parts.html itself, further down.
function bundleFile(name, data) {
  return '<script>\n// Generated by tools/SpriteBundle. Do not edit by hand.\n' +
         'var GOOSE_SPRITES = GOOSE_SPRITES || {};\nGOOSE_SPRITES["' + name + '"] = ' +
         JSON.stringify(data, null, 2) + ';\n</script>\n';
}

const BUNDLE_FILES = { 'sprites-parts': 'parts', 'sprites-effects': 'effects' };
...
function makeServer(sheets, options) {
  ...
  const server = {
    ...
    // The lazily fetched sprite bundles. `includeFailsOn` refuses one file (a transient Apps Script
    // error, or a deploy missing the file); `includeGarbageOn` answers with something that is not a
    // bundle, which is the failure a parse has to survive rather than accept.
    include(file) {
      if (opts.includeFailsOn === file) throw new Error('include boom: ' + file);
      if (opts.includeGarbageOn === file) return '<script>// nothing useful here\n</script>';
      if (opts.includePayload && opts.includePayload[file]) return opts.includePayload[file];
      const name = BUNDLE_FILES[file];
      if (!name || !SERVED[name]) throw new Error('no HTML file named "' + file + '"');
      return bundleFile(name, SERVED[name]);
    },
...
test('a bundle that never arrives at all still leaves the form usable', () => {
  // The deployment with no sprites-parts.html in it: every fetch for it fails, and the form has to
  // render anyway — every field but the art is editable, and the art was never the urgent half.
  const saved = SERVED.parts;
  try {
    delete SERVED.parts;
...
test('a bundle that fails to decode is reported and does not block the sheet', () => {
  ...
  assert.match(h.status(), /1 records — Failed to load the icons sprite bundle/);
...
test('a failed bundle decode is cached — record opens render at once, with no re-decode', () => {
  ...
  // Nothing is decoding YET: parts is not on the page, so it has to be fetched first.
  assert.equal(h.img.pending.length, 0);
  assert.deepEqual(serverCalls(h.run, 'include').map((c) => c.args), [['sprites-parts']]);
  h.run.flush();
...
// --- the two bundles the page does not carry -------------------------------------------------

test('no sheet pays for parts or effects until a record that needs them is opened', () => {
  // The whole point of not inlining them: 2.8MB of base64 that 17 of the 21 sheets never draw from,
  // re-served on every sidebar open. Opening a sheet — even Items, which does need parts — asks for
  // nothing until a record is actually on screen.
  const h = boot({ Items: [ITEM(1, 'Gold')], 'NPC Drops': [DROP(1, 1)] });
  assert.deepEqual(serverCalls(h.run, 'include'), []);
  ...
  // NPC Drops draws no character and no animation, so it never asks — bundlesFor is what knows.
  assert.deepEqual(App.bundlesFor(schemaOf('NPC Drops')), ['icons']);
  assert.deepEqual(serverCalls(h.run, 'include'), []);
});

test('a fetched bundle is fetched ONCE — per record, per sheet, per session', () => {
  const h = boot({ Items: [ITEM(1, 'Gold'), ITEM(2, 'Sword')], NPCs: [NPC(1, 'Rat')] });

  fire(h.get('records').children[0], 'click');
  h.settle();
  assert.deepEqual(serverCalls(h.run, 'include').map((c) => c.args), [['sprites-parts']]);
  assert.ok(App.__state.images.parts, 'and it decoded');
  ...
  assert.equal(serverCalls(h.run, 'include').length, 1, 'the bundle is in hand — nothing re-asks');
});

test('two records opened before the reply lands ask the server once', () => {
  // The waiter queue spans the FETCH now, not just the decode: without that, every record opened
  // while a 2MB reply is in flight starts a fetch of its own.
  const h = boot({ Items: [ITEM(1, 'Gold'), ITEM(2, 'Sword')] });

  fire(h.get('records').children[0], 'click');
  fire(h.get('records').children[1], 'click');
  assert.equal(serverCalls(h.run, 'include').length, 1);

  h.settle();
  assert.equal(serverCalls(h.run, 'include').length, 1);
  assert.equal(h.img.pending.length, 0, 'and one decode, already done');
});

test('the form says the art is on its way, and gives the line back afterwards', () => {
  // The only other symptom of a multi-megabyte fetch is a blank canvas, which reads as broken art
  // rather than as art arriving. What it replaces — the record count — is put back, not lost.
  const h = boot({ Items: [ITEM(1, 'Gold', { graphic_equip: 5, item_slot: 'Helmet' })] });
  assert.equal(h.status(), '1 records');

  fire(h.get('records').children[0], 'click');
  assert.equal(h.status(), 'Fetching parts art…');

  h.settle();
  assert.equal(h.status(), '1 records');
});

test('a message that arrives while the art is in flight is not overwritten by the restore', () => {
  // The line the fetch covered is stale news by then: a save confirmation is what the user is
  // waiting to read, and putting the record count back over it would look like the save vanished.
  const h = boot({ Items: [ITEM(1, 'Gold')] });
  fire(h.get('records').children[0], 'click');
  assert.equal(h.status(), 'Fetching parts art…');
...
test('a bundle the server refuses is reported once and not re-fetched per record', () => {
  const h = boot({ Items: [ITEM(1, 'Gold'), ITEM(2, 'Sword')] },
    { includeFailsOn: 'sprites-parts' });

  fire(h.get('records').children[0], 'click');
  h.settle();
  assert.match(h.status(), /Failed to load the parts sprite bundle/);
  assert.equal(h.get('status').className, 'error');
  ...
  h.settle();
  assert.equal(serverCalls(h.run, 'include').length, 1,
               'a failure is a RESULT — retrying it per record open is what the cache prevents');
  // And the record still renders, with the warning still on screen rather than buried.
  assert.equal(h.get('form').querySelector('[name="item_name"]').value, 'Sword');
  assert.match(h.status(), /Failed to load the parts sprite bundle/);
});

test('a reply that is not a bundle is a failure, not silently no art', () => {
  // A deploy serving something else under that name would otherwise leave every canvas on the sheet
  // blank with nothing on the status line to explain it.
  const h = boot({ Items: [ITEM(1, 'Gold')] }, { includeGarbageOn: 'sprites-parts' });
  fire(h.get('records').children[0], 'click');
  h.settle();

  assert.match(h.status(), /Failed to load the parts sprite bundle/);
  assert.ok(!App.__state.bundles.parts, 'and nothing half-parsed was kept');
  assert.equal(h.get('form').querySelector('[name="item_name"]').value, 'Gold');
});

test('the REAL generated bundles parse to exactly what their script tag assigns', () => {
  // The one thing the fake payloads cannot check: that tools/SpriteBundle still writes a literal
  // JSON.parse can read. If it ever emits a trailing comma, a comment inside the object, an unquoted
  // key or a second statement after it, this fails here rather than in a deployed sidebar with every
  // character sprite blank. The truth is the file EXECUTED, the way Editor.html used to run it.
  const truth = (() => {
    const ctx = vm.createContext({});
    for (const name of ['parts', 'effects']) {
      const html = readFileSync(fileURLToPath(new URL(`../sprites-${name}.html`, import.meta.url)),
                                'utf8');
  ...
  const payload = {};
  ['parts', 'effects'].forEach((name) => {
    payload['sprites-' + name] = readFileSync(
      fileURLToPath(new URL(`../sprites-${name}.html`, import.meta.url)), 'utf8');
  });

  const h = boot({ 'Spell Effects': [rowFor('Spell Effects', {
    spell_effect_id: 1, spell_effect_name: 'Flame', effect_type: 'Instant', spell_effected: 'Anyone',
  })] }, { includePayload: payload });
  h.get('sheet-picker').value = 'Spell Effects';
  fire(h.get('sheet-picker'), 'change');
  h.settle();
  fire(h.get('records').children[0], 'click');
  h.settle();

  // Spell Effects is the one sheet that asks for both.
  assert.deepEqual(App.bundlesFor(schemaOf('Spell Effects')), ['icons', 'parts', 'effects']);
  ['parts', 'effects'].forEach((name) => {
    assert.ok(Object.keys(truth[name].rects).length > 1000, name + ' fixture looks wrong');
    // JSON, not deepEqual: `truth` was built inside a vm context, so its objects carry that realm's
    // prototypes and deepEqual — which compares them — refuses a structurally identical result.
    assert.equal(JSON.stringify(App.__state.bundles[name]), JSON.stringify(truth[name]),
                 name + ': the parsed bundle is not what the file assigns');
  });
  assert.doesNotMatch(h.status(), /Failed to load/);
});
...
test('the NPC preview uses the row\'s body_state to pick the clip (#15)', () => {
  ...
  const saved = SERVED.parts.rects;
  try {
    SERVED.parts.rects = {
...
test('the effect preview canvas is sized by the scale its context is given', () => {
  ...
  try {
    ...
    SERVED.effects.rects = { '77:0': [0, 0, 16, 16] };
    const h = boot({
...
test('changing spell_effect_id restarts the effect animation live', () => {
  ...
  try {
    SERVED.effects.rects = { '77:0': [0, 0, 16, 16], '88:0': [16, 0, 16, 16] };
    const h = boot({
      Spells: [rowFor('Spells', { spell_id: 1, spell_name: 'Fire', spell_target: 'Self',
                                  spellbook_graphic: 1, spell_effect_id: 4 })],
      'Spell Effects': [rowFor('Spell Effects', {
...
test('the Spells preview draws the row\'s ANIMATION, not the row id, when both exist in the atlas', () => {
  ...
  try {
    ...
    SERVED.effects.rects = { '4:0': [0, 0, 16, 16], '77:0': [16, 0, 16, 16] };
    const h = boot({
      Spells: [rowFor('Spells', { spell_id: 1, spell_name: 'Fire', spell_target: 'Self',
                                  spellbook_graphic: 1, spell_effect_id: 4 })],
      'Spell Effects': [rowFor('Spell Effects', {
...
test('a spell_effect_id naming no known row shows no effect panel rather than a guess', () => {
  SERVED.effects.rects = { '4:0': [0, 0, 16, 16] };
  try {
    const h = boot({
      Spells: [rowFor('Spells', { spell_id: 1, spell_name: 'Fire', spell_target: 'Self',
                                  spellbook_graphic: 1, spell_effect_id: 4 })],
...
test('a graphic nothing can be checked against does not block the save', () => {
  ...
  // The icons bundle is the inlined one, so "it did not load" is it being absent from the page.
