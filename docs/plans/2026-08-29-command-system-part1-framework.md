...
- `CommandContext`: **`public sealed class CommandContext`** (public because separately compiled `.csx` script handlers name it in their signatures). **Construction:** internal constructor `internal CommandContext(Player player, GameWorld world, CommandRegistry registry, string[] args, string remainder)` — dispatch constructs it **immediately after parsing `Args`/`Remainder` (invoker step 2), before `CheckAccess`**, so every later step (including `CheckAccess` and the handlers) receives the same instance. Members: `Player Player`, `GameWorld World`, `CommandRegistry Registry`, `string[] Args` (tokens after the command key, split on `' '` — the protocol's single-space delimiter — with empty entries removed; **includes the subcommand token**: the binder receives only the tokens after it, per the invoker's step 6, while `CheckAccess` and handlers see the full array), `string Remainder` (the raw packet text after the key, lossless), `public string Usage { get; internal set; } = ""` (the selected target's precomputed usage line, override applied — for parse-error replies and in-body empty-input guards; **public getter so scripts can read it, internal setter so only dispatch assigns it** — assigned after target selection in invoker step 6), `void Send(string message)` → `World.Send(Player, P.ServerMessage(message))`.
