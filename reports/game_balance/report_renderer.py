...
def render_report(data: Any, summary: dict[str, Any]) -> str:
    ...
    npc_outliers = []
    npc_rule_outlier_ids = set()
    npc_curated_peer_ids = set()
    curated_npc_ids = {20, 24, 28, 40, 169}
    for npc_id in sorted(data.leveling_npc_ids):
        ...
        matches_rule = hp_ratio >= 1.8 or hp_ratio <= 0.6 or xp_ratio >= 1.8 or xp_ratio <= 0.6
        if matches_rule:
            npc_rule_outlier_ids.add(npc_id)
        elif npc_id in curated_npc_ids:
            npc_curated_peer_ids.add(npc_id)
        if matches_rule or npc_id in curated_npc_ids:
            reason = "threshold match" if matches_rule else "curated comparison"
            npc_outliers.append((npc_id, data.npcs[npc_id]["npc_name"], metric.level, n(metric.hp), n(metric.ac), n(data.npcs[npc_id]["experience"]), n(hp_ratio, 2), n(xp_ratio, 2), n(metric.xp_per_hp, 3), reason))
    ...
    one_time_quests = [
        ...
        if quest_id in data.effective_quest_levels and str(quest["repeatable"]) == "0"
    ]
    ...
    for quest_id in sorted(data.effective_quest_levels):
        ...
        if expected_kills is not None:
            quest_workloads.append((expected_kills, quest_id, str(quest["name"])))
    ...
    anomaly_rows = []
    elemental = {row["rank"]: row for row in spells if row["family"] == "Elemental Strike"}
    if 8 in elemental and 9 in elemental:
        rank_8 = elemental[8]
        rank_9 = elemental[9]
        if rank_8["magnitude"] is not None and rank_9["magnitude"] is not None:
            anomaly_rows.append(("High", "Elemental Strike", f"Rank 8 magnitude {n(rank_8['magnitude'], 2)} at {n(rank_8['cooldown'], 1)}s; rank 9 magnitude {n(rank_9['magnitude'], 2)} at {n(rank_9['cooldown'], 1)}s", "Review the adjacent rank cost/cooldown inversion when the later rank is stronger and faster"))
    for family, priority in (("Backstab", "High"), ("Fortify", "Medium")):
        family_rows = sorted(grouped_spells.get(family, ()), key=lambda row: (row["rank"] if row["rank"] is not None else 0, row["id"]))
        cooldowns = [row["cooldown"] for row in family_rows]
        if len(cooldowns) >= 2 and any(after < before for before, after in zip(cooldowns, cooldowns[1:])):
            anomaly_rows.append((priority, family, "Cooldowns " + " / ".join(n(value, 1) for value in cooldowns) + " seconds", "Cooldown progression decreases at a later rank; verify that the reversal is intentional"))
    anomaly_rows.append(("High", "Late scroll sourcing", f"{len(late_spells)} nominal L39–49 spells resolve at L45 or L50", ", ".join(f"{row['name']} {row['nominal']}→{row['practical']}" for row in late_spells) or "No rows"))
    item_outlier_rows = []
    selected_outlier_ids = {451, 18, 214, 132, 140, 161, 163, 115}
    for row in item_rows:
        item_id = row["id"]
        item = data.items[item_id]
        metric_name = "throughput" if row["use"] == "Weapon" else "budget"
        classes = set(allowed_classes(item["class_restrictions"], data.classes))
        earlier = [
            candidate
            for candidate in item_rows
            if candidate["use"] == row["use"]
            and candidate["slot"] == row["slot"]
            and candidate["effective"] < row["effective"]
            and classes.intersection(allowed_classes(data.items[candidate["id"]]["class_restrictions"], data.classes))
        ]
        dominator = max(earlier, key=lambda candidate: candidate["metrics"][metric_name], default=None)
        finding = "Selected legacy anchor"
        if dominator is not None and dominator["metrics"][metric_name] > row["metrics"][metric_name] * 1.05:
            selected_outlier_ids.add(item_id)
            finding = f"Trails {dominator['name']} #{dominator['id']} at L{dominator['effective']} ({n(dominator['metrics'][metric_name], 2)} {metric_name})"
        if item_id not in selected_outlier_ids:
            continue
        item_outlier_rows.append((item_id, item["item_name"], row["effective"], row["slot"], n(row["metrics"]["throughput"], 2), n(row["metrics"]["budget"], 2), n(row["metrics"]["effective_hp"]), finding, source_text(data, item_id, True)))
    anchors = []
    anchor_specs = (
    ...
    for label, slot_id, target, class_id, metric_name in anchor_specs:
        ...
        if prior is None or following is None or not prior["effective"] < target < following["effective"]:
            continue
    ...
    equipment_summary = summary["items"]
    pre_50_npc_count = sum(data.npc_metrics[npc_id].level < 50 for npc_id in data.leveling_npc_ids)
    accessory_slots = (4, 5, 6, 7, 8, 9, 13)
    accessory_evidence = ", ".join(
        f"{SLOT_NAMES[slot_id]} L{slot_levels[slot_id][0]}"
        if slot_levels[slot_id]
        else f"{SLOT_NAMES[slot_id]} missing"
        for slot_id in accessory_slots
    )
    heaviest_quests = sorted(quest_workloads, reverse=True)[:2]
    workload_evidence = "; ".join(
        f"Q{quest_id} {name} ≈{n(expected, 1)} expected kills"
        for expected, quest_id, name in heaviest_quests
    ) or "No modeled kill workload"
    threshold_49 = thresholds.get(49, 0)
    authored_49 = sum(value for level, value in quest_xp_by_authored_level.items() if level <= 49)
    practical_49 = sum(value for level, value in quest_xp_by_level.items() if level <= 49)
    authored_49_pct = authored_49 / threshold_49 * 100 if threshold_49 else 0
    practical_49_pct = practical_49 / threshold_49 * 100 if threshold_49 else 0
    priority_rows = [
        ("P0", "Equipment outliers", "Practice Katana #451, Searing Whip #214, and the unrestricted Poo Flinger set remain on the selected-outlier audit", "Reprice/relevel or trim legacy vectors before adding stronger successors"),
        ("P1", "Accessory progression", accessory_evidence, "Coverage is now broad; protect the roughly ten-level ladders and check each source-level slip"),
        ("P1", "NPC same-level fairness", f"{len(npc_rule_outlier_ids)} threshold matches plus {len(npc_curated_peer_ids)} curated peer comparisons", "Normalize XP to effective durability and pressure"),
        ("P1", "Quest workload", workload_evidence, "Raise drop rates, lower counts, or raise durable rewards for the heaviest grinds"),
        ("P1", "Quest coverage", f"One-time raw XP through L49 is {n(authored_49_pct, 1)}% by authored level and {n(practical_49_pct, 1)}% by practical level", "Use practical dependency levels when placing future quest XP"),
    ]
    if 140 in data.effective_item_levels and 666 in data.effective_item_levels:
        poo_budget = item_metrics(data.items[140])["budget"]
        exalted_budget = item_metrics(data.items[666])["budget"]
        if poo_budget > exalted_budget:
            priority_rows.insert(1, ("P0", "Glove frontier inversion", f"Gloves of the Poo Flinger #140 at L{data.effective_item_levels[140]} has budget {n(poo_budget, 1)} versus Exalted War Gloves #666 at L{data.effective_item_levels[666]} with {n(exalted_budget, 1)}", "Reduce the legacy outlier or make the intended late glove a real upgrade"))
    if anomaly_rows:
        spell_priority = "P0" if any(row[0] == "Critical" for row in anomaly_rows) else "P1"
        priority_rows.append((spell_priority, "Spell pacing", f"{len(anomaly_rows)} anomaly rows remain, including {anomaly_rows[0][1]}", "Review the detailed rank-growth and cooldown evidence"))
    tables = []
    tables.append(render_table("table-priorities", "Prioritized interventions", ["Priority", "Area", "Evidence", "Recommended action"], priority_rows))
    ...
    tables.append(render_table("table-equipment", "Focused included equipment detail", ["ID", "Name", "Kind", "Slot", "Type", "Classes", "Authored L", "Effective L", "Raw vector", "Effective HP", "Effective MP", "Dodge pp", "Weapon throughput", "Budget", "Frontier", "Eligible sources"], [(row["id"], row["name"], row["use"], row["slot"], data.items[row["id"]]["item_type"], row["classes"], row["authored"], row["effective"], row["stats"], n(row["metrics"]["effective_hp"]), n(row["metrics"]["effective_mp"]), n(row["metrics"]["dodge_pct_points"], 2), n(row["metrics"]["throughput"], 2), n(row["metrics"]["budget"], 2), row["frontier"], row["sources"]) for row in item_rows], f"All {len(item_rows)} modeled-source equipment templates; non-equipment remains in the full included-item audit."))
    tables.append(render_table("table-items", "Full included-item audit", ["ID", "Name", "Use type", "Slot", "Type", "Classes", "Authored L", "Effective L", "Raw vector", "Effective HP", "Effective MP", "Dodge pp", "Weapon throughput", "Budget", "Frontier", "Eligible sources"], included_item_table_rows, f"All {len(included_item_table_rows)} templates with a valid modeled acquisition chain."))
    tables.append(render_table("table-item-outliers", "Equipment regressions and selected anchors", ["ID", "Item", "Effective L", "Slot", "Throughput", "Budget", "Effective HP", "Finding", "Source"], item_outlier_rows, "Includes selected legacy anchors plus any item that trails a substantially stronger earlier item in the same slot and overlapping class scope."))
    tables.append(render_table("table-excluded-equipment", "Focused excluded equipment audit", ["ID", "Name", "Authored L", "Min XP", "Slot", "Classes", "Reason", "All modeled paths"], [(item_id, data.items[item_id]["item_name"], data.items[item_id]["min_level"], data.items[item_id]["min_experience"], SLOT_NAMES.get(int(data.items[item_id]["item_slot"]), data.items[item_id]["item_slot"]), class_label(data.items[item_id]["class_restrictions"], data.classes), data.excluded_item_reasons[item_id], source_text(data, item_id)) for item_id in excluded_gear], f"All {len(excluded_gear)} scoped no-XP equipment templates without a valid modeled source."))
    tables.append(render_table("table-excluded-items", "Full excluded-item audit", ["ID", "Name", "Use type", "Authored L", "Min XP", "Slot", "Classes", "Reason", "All modeled paths"], [(item_id, data.items[item_id]["item_name"], data.items[item_id]["item_usetype"], data.items[item_id]["min_level"], data.items[item_id]["min_experience"], SLOT_NAMES.get(int(data.items[item_id]["item_slot"]), data.items[item_id]["item_slot"]), class_label(data.items[item_id]["class_restrictions"], data.classes), data.excluded_item_reasons[item_id], source_text(data, item_id)) for item_id in sorted(data.excluded_item_reasons)], f"All {len(data.excluded_item_reasons)} excluded templates, including direct-scope source failures and templates outside the level/XP scope; premium-only and XP-gated-only paths remain visible."))
    ...
    tables.append(render_table("table-npc-outliers", "NPC same-level outliers", ["ID", "NPC", "Level", "HP", "AC", "Raw XP", "HP / median", "XP / median", "XP / HP", "Reason"], npc_outliers))
    ...
    threshold_10 = thresholds.get(10, 0)
    authored_10 = sum(value for level, value in quest_xp_by_authored_level.items() if level <= 10)
    practical_10 = sum(value for level, value in quest_xp_by_level.items() if level <= 10)
    authored_10_pct = authored_10 / threshold_10 * 100 if threshold_10 else 0
    practical_10_pct = practical_10 / threshold_10 * 100 if threshold_10 else 0
    top_mismatches = sorted(quest_rows, key=lambda row: (-abs(row[5]), row[0]))[:3]
    mismatch_evidence = "; ".join(
        f"Q{row[0]} {row[3]}→{row[4]}"
        for row in top_mismatches
        if row[5]
    ) or "No authored/practical mismatch"
    automatic_grants = ", ".join(
        f"{data.classes[class_id]['class_name']} {auto_counts[data.classes[class_id]['class_name']]}"
        for class_id in PLAYER_CLASS_IDS
    )
    css = """
...
<section id=executive><h2>Executive findings</h2><div class=cards><div class=card><strong>{summary['items']['total']}</strong>item templates</div><div class=card><strong>{summary['items']['in_level_xp_scope']}</strong>direct-scope items</div><div class=card><strong>{summary['items']['included']}</strong>modeled eligible chains</div><div class=card><strong>{equipment_summary['no_xp_equipment']}</strong>no-XP equipment templates</div><div class=card><strong>{equipment_summary['valid_source_equipment']}</strong>valid modeled sources</div><div class=card><strong>{equipment_summary['pre_50_equipment']} / {equipment_summary['first_available_at_50_equipment']}</strong>effective &lt;50 / =50</div><div class=card><strong>{equipment_summary['no_valid_source_equipment']}</strong>no valid source</div><div class=card><strong>{pre_50_npc_count} / {len(data.transition_npc_ids)} / {len(data.endgame_npc_ids)}</strong>pre-50 / ungated L50 / gated L50 monsters</div><div class=card><strong>{len(one_time_quests)}</strong>reachable one-time quests</div><div class=card><strong>{n(total_raw_xp)}</strong>one-time raw quest XP</div><div class=card><strong>{n(total_gold)}</strong>one-time quest gold</div><div class=card><strong>{len(data.spells)}</strong>spell templates</div></div>
<div class=callout><strong>Accessory coverage is substantially improved.</strong> Current first effective levels are {e(accessory_evidence)}. Every equipment slot now has a modeled source, and the added accessory ladders generally land about ten levels apart.</div>
<div class='callout danger'><strong>Legacy outliers still distort the new ladders.</strong> The Poo Flinger gloves at L40 remain stronger on the disclosed budget proxy than the L44 Exalted War Gloves, while other selected weapon and armor inversions remain in the detail audit.</div>
<div class=callout><strong>Combat rewards still need review.</strong> {len(npc_rule_outlier_ids)} leveling NPCs trigger the same-level HP/XP threshold rule, with {len(npc_curated_peer_ids)} additional peer comparisons retained for context. The exact templates, effective durability, pressure, and XP efficiency are listed below.</div>
{tables[0]}</section>
<section id=items><h2>Item acquisition, scaling, and gaps</h2><p>Effective level is the earliest eligible source, bounded by the item requirement. Monster drops use the monster/map gate; vendors use the earliest eligible map gate; quest rewards use practical quest level; crafts recursively require the recipe and every ingredient. The current split is {equipment_summary['no_xp_equipment']} scoped equipment templates, {equipment_summary['valid_source_equipment']} with valid modeled sources, {equipment_summary['pre_50_equipment']} available before L50, {equipment_summary['first_available_at_50_equipment']} first available at L50, and {equipment_summary['no_valid_source_equipment']} without a valid source.</p><div class=charts>{charts[0]}{charts[1]}{charts[2]}</div>{''.join(tables[1:10])}</section>
<section id=npcs><h2>NPC scaling and outliers</h2><p>There are {pre_50_npc_count} L1–49 monster templates, {len(data.transition_npc_ids)} non-XP-gated L50 transition templates, and {len(data.endgame_npc_ids)} gated-only L50 endgame templates. Missing exact monster levels are {e(', '.join(map(str, missing_levels)) or 'none')}. Level 50 is a mixed regime and is reported separately rather than included in the pre-50 median curve.</p><div class=charts>{charts[3]}{charts[4]}{charts[8]}</div>{''.join(tables[10:13])}</section>
<section id=quests><h2>Quest economy and coverage</h2><p>The {len(one_time_quests)} reachable one-time quests award {n(total_raw_xp)} raw XP and {n(total_gold)} gold. By authored minimum, raw one-time XP reaches {n(authored_10_pct, 1)}% of the cumulative Rogue threshold through L10 and {n(authored_49_pct, 1)}% through L49. Dependency-adjusted practical coverage is {n(practical_10_pct, 1)}% and {n(practical_49_pct, 1)}% at those anchors.</p><div class=callout><strong>Largest authored-to-practical shifts:</strong> {e(mismatch_evidence)}. <strong>Heaviest modeled workloads:</strong> {e(workload_evidence)}.</div><div class=charts>{charts[5]}{charts[6]}</div>{''.join(tables[13:16])}</section>
<section id=spells><h2>Spell pacing and anomalies</h2><p>Automatic grants are {e(automatic_grants)}. Scroll/quest access uses a class floor of L5. Resource percentage uses a full-resource authoring model: static cost is removed first, then percent cost is truncated from the remainder; the table reports the largest eligible-class HP/MP share at practical unlock and is not a full cast simulation. Rank growth uses absolute constants, comparable linear coefficients, single direct fields, and per-component deltas for stable multi-stat vectors. Truly scripted effects without numeric fields, nonlinear formulas, multiple formula channels, and changed component sets remain explicitly N/A. Delta and ratio are calculated only when adjacent ranks share a comparable magnitude basis or component set, with duration and target geometry kept separate.</p><div class=charts>{charts[7]}</div>{''.join(tables[16:20])}</section>
